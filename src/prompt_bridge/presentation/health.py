...
class HealthRoutes:
    ...
    async def health_check(self, request: Request) -> HealthResponseDTO:
        ...
        # Get basic stats
        session_pool_stats = self._session_pool.get_stats() if self._session_pool else None
        providers = self._registry.list_providers() if self._registry else None
        provider_health = await self._registry.health_check_all() if self._registry else None
        circuit_breakers = self._chat_use_case.get_circuit_breaker_status() if self._chat_use_case else None
    ...
    async def detailed_health(self) -> DetailedHealthResponseDTO:
        ...
        all_providers_healthy = all(provider_health.values()) if provider_health else True
        pool_healthy = pool_stats.get("available", 0) > 0 if pool_stats else True

        overall_status = "healthy" if (all_providers_healthy and pool_healthy) else "degraded"
